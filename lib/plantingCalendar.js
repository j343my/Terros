...
const INCOMPATIBILITES_PLANTES = {
  tomate: {
    pomme_de_terre: 'Risque commun de mildiou (éviter la proximité).',
    fenouil: 'Le fenouil freine la croissance de la tomate.',
  },
  tomate_cerise: {
    pomme_de_terre: 'Risque commun de mildiou (éviter la proximité).',
    fenouil: 'Le fenouil freine la croissance de la tomate.',
  ...
  concombre: {
    pomme_de_terre: 'Concurrence forte + risque de maladies cryptogamiques.',
    sauge: 'La sauge gêne la croissance du concombre.',
  },
  courgette: {
    pomme_de_terre: 'Concurrence forte + risque de maladies cryptogamiques.',
  },
  haricot_vert: {
    oignon_blanc: 'Les alliacées limitent le développement des haricots.',
    echalote: 'Les alliacées limitent le développement des haricots.',
    ail: 'Les alliacées limitent le développement des haricots.',
  },
  pois: {
    oignon_blanc: 'Les alliacées limitent le développement des pois.',
    echalote: 'Les alliacées limitent le développement des pois.',
    ail: 'Les alliacées limitent le développement des pois.',
  },
  laitue: {
    persil: 'Compétition racinaire en bac compact.',
  },
};

const FAMILLE_INCOMPATIBILITES = {
  solanacee: {
    solanacee: 'Même famille : pression maladies/ravageurs plus élevée en bac.',
  },
  cucurbitacee: {
    cucurbitacee: 'Même famille : concurrence forte en eau et nutriments.',
  },
  legumineuse: {
    legume_bulbe: 'Légumineuses + bulbes (alliacées) : association souvent peu productive.',
    bulbe: 'Légumineuses + alliacées : association souvent peu productive.',
  },
  legume_bulbe: {
    legumineuse: 'Alliacées + légumineuses : association souvent peu productive.',
  },
  bulbe: {
    legumineuse: 'Alliacées + légumineuses : association souvent peu productive.',
  },
...
const toMapKey = (name) => normalizePlantKey(name).replace(/'/g, '').replace(/ /g, '_');

/**
 * Détecte les incompatibilités de voisinage en bac pour une plante.
 * @returns {Array<{nom: string, raison: string}>}
 */
export function getIncompatibilitesDansBac(planteNom, plantesDansBac) {
  const ref = findPlante(planteNom);
  const sourceKey = toMapKey(planteNom);
  const sourceRules = INCOMPATIBILITES_PLANTES[sourceKey] || {};
  const familyRules = ref ? FAMILLE_INCOMPATIBILITES[toMapKey(ref.famille)] : null;
  const sourceFamily = ref ? toMapKey(ref.famille) : null;

  const conflicts = (plantesDansBac || []).flatMap(plant => {
    const targetRef = findPlante(plant.nom);
    const targetKey = toMapKey(plant.nom);
    const targetFamily = targetRef ? toMapKey(targetRef.famille) : null;

    const direct = sourceRules[targetKey];
    if (direct) return [{ nom: plant.nom, raison: direct }];

    const reverseRules = INCOMPATIBILITES_PLANTES[targetKey] || {};
    const reverse = reverseRules[sourceKey];
    if (reverse) return [{ nom: plant.nom, raison: reverse }];

    if (familyRules && targetRef) {
      const familyReason = familyRules[targetFamily];
      if (familyReason) return [{ nom: plant.nom, raison: familyReason }];
    }

    if (sourceFamily && targetFamily && sourceFamily === targetFamily) {
      return [{ nom: plant.nom, raison: 'Même famille botanique dans le même bac : surveiller maladies et épuisement du substrat.' }];
    }
    return [];
  });
  ...
  conflicts.forEach(item => {
    const key = `${item.nom}::${item.raison}`;
